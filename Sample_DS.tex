...
\title{Diagnostic Inflation Expectations in China:\\ The Role of Uncertainty and Geopolitical Risk}
\author{Jingle Fu}
\date{\today}
...
\begin{abstract}
    This paper tests the diagnostic expectations hypothesis within the unique context of Chinese household inflation forecasts. Utilizing quarterly survey data (2013–2025) and a novel combination of Bayesian time-varying parameter and sign-restricted VAR methodologies, we document a robust and pervasive pattern of overreaction: forecast revisions are systematically followed by forecast errors of the opposite sign. The intensity of this diagnostic bias intensifies during periods of heightened economic policy uncertainty and geopolitical risk, such as the 2015-16 RMB volatility and the 2018-19 trade war. Crucially, we address fundamental identification challenges by constructing a measure of actual inflation that aligns with the survey's quarterly horizon and by identifying a structural "diagnostic news shock" that not only drives a wedge between expected and realized inflation but also aligns with the theoretical predictions of the diagnostic framework regarding output and uncertainty dynamics. This shock explains a substantial share of short-term expectation volatility but has limited influence on actual inflation outcomes. Our findings highlight the behavioral foundations of expectation formation in a major emerging economy and underscore the nuanced challenges facing monetary communication, particularly when expectations are prone to salience-driven overreaction.
\end{abstract}
...
In practice, a calibration for $\delta$ is needed. The literature often either assumes a fixed indifference interval or estimates it from historical data. We follow common practice by choosing $\delta = 0.5$ percentage points, reflecting that respondents view price moves within $\pm 0.5$ percentage points as essentially "unchanged." This choice is in line with other survey quantifications and provides a reasonable fit to historical revisions. Our results are not sensitive to moderate variations in $\delta$—sensitivity analysis with $\delta \in \{0.3, 0.4, 0.5, 0.6, 0.7\}$ yields qualitatively similar patterns of diagnostic bias.
...
A critical issue in testing forecast accuracy is ensuring the forecast horizon of the expectation measure matches that of the realized value. The PBoC survey explicitly asks about price changes over the **next quarter**. The Carlson-Parkin conversion yields a number $\mu_t$ that we interpret as the mean expected **quarterly inflation rate** (expressed in percentage points).

To construct a commensurate measure of realized inflation, we cannot use the standard year-on-year (YoY) CPI growth rate, as it compares prices to those twelve months prior. Instead, we require a measure of the inflation rate that actually materialized over the specific quarter that households were forecasting. Therefore, we construct $\pi_{t+1}$ as the **quarter-on-quarter annualized rate** of CPI inflation for quarter $t+1$. This is calculated from seasonally adjusted monthly CPI indices: $ \pi_{t+1}^{QoQ-ann} = [(CPI_{t+1,end}/CPI_{t,end})^4 - 1] \times 100$, where $CPI_{t,end}$ is the index level at the end of quarter $t$. This measure directly answers the question: "What was the annualized rate of price increase during the quarter you were forecasting?"

This alignment is essential for a clean test of diagnostic expectations. The forecast error is thus defined as $FE_t = \pi_{t+1}^{QoQ-ann} - \mu_t$, and the forecast revision as $FR_t = \mu_t - \mu_{t-1}$. Our final estimation sample, after ensuring data availability for these constructed series, runs from 2013Q4 to 2025Q2, yielding 43 quarterly observations.
...
Table \ref{tab:desc_stats} presents summary statistics. Several features stand out. First, households, on average, systematically over-predict inflation: the mean forecast error $FE_t$ is -0.31 percentage points, indicating actual inflation turned out about 0.31 percentage points lower than expected on average. This upward bias in expectations aligns with international findings that consumers often overestimate inflation. In China's case, this could reflect households recalling periods of higher inflation or focusing on frequently rising items (like food prices) more than on stable or falling prices. Second, expected inflation is significantly more volatile than realized inflation (standard deviation of 0.55 vs. 0.27), suggesting that subjective perceptions swing more widely than realized prices—likely reflecting the influence of headline events and media on expectations. Third, the uncertainty and risk indices exhibit large variation (EPU in particular varies by a factor of six from trough to peak), mirroring the turbulent economic and geopolitical environment of the 2013-2025 period.

Most importantly, the simple correlation between $FE_t$ and $FR_t$ is -0.45, a strikingly strong negative correlation providing initial evidence that when expectations go up, subsequent errors tend to be negative, and vice versa. A basic OLS regression (Table \ref{tab:ols_baseline}) yields a coefficient of -0.54 (standard error 0.17), highly significant and providing preliminary evidence consistent with diagnostic overreaction. This regression, albeit simple, foreshadows our main results: a negative coefficient on forecast revisions is exactly what diagnostic expectations theory predicts.

\begin{table}[!htbp]
    \centering
    \caption{Summary Statistics (2013Q4–2025Q2)}
    ...
    \begin{threeparttable}
        \begin{tabular}{lcccc}
            ...
            \midrule
            Expected inflation, $\mu_t$ (\%)                & 0.45  & 0.55      & -0.01 & 0.54  \\
            Realized inflation (QoQ ann.), $\pi_{t+1}$ (\%) & 0.14  & 0.27      & -0.30 & 0.54  \\
            Forecast error, $FE_t$ (\%)                     & -0.31 & 0.38      & -1.12 & 0.65  \\
            Forecast revision, $FR_t$ (\%)                  & -0.03 & 0.43      & -1.05 & 1.02  \\
            EPU index                                       & 225.3 & 112.9     & 75.9  & 499.2 \\
            GPR index                                       & 105.0 & 29.1      & 69.7  & 224.6 \\
            \bottomrule
        \end{tabular}
        \begin{tablenotes}
            \footnotesize
            \item Note: Inflation rates in percentage points. EPU and GPR are unitless indices. Sample: 43 quarters.
        \end{tablenotes}
    \end{threeparttable}
\end{table}
...
Table \ref{tab:corr_matrix} presents the pairwise correlations among our key variables. The quantified expectation $\mu_t$ correlates positively with actual CPI inflation ($r=0.54$), but this correlation is far from perfect, confirming that expectations contain substantial variation independent of actual outcomes. The negative correlation between $FE_t$ and forecast revision $FR_t$ ($r=-0.45$) is the key diagnostic signature. EPU shows a moderately negative correlation with $\mu_t$ ($r=-0.46$), possibly because EPU surges often coincide with deflation fears or demand collapse scenarios. GPR's correlations are generally smaller in magnitude, suggesting geopolitical risk has more idiosyncratic effects not strongly captured by linear associations.

\input{outputs/tables/correlation_matrix.tex}

Table \ref{tab:subperiod} examines whether diagnostic expectations behavior differs across distinct macroeconomic regimes by splitting the sample into pre-2018 and 2018-onward subperiods. The year 2018 marks an important structural break when U.S.-China trade tensions escalated sharply. Comparing the two subperiods, we observe striking differences. Quantified expectations $\mu_t$ averaged 0.79 percentage points in 2011-2017 versus 0.25 in 2018-2025, reflecting that Chinese inflation has been generally lower and more stable in recent years. More telling is the behavior of forecast errors: in the earlier subperiod, the mean $FE$ was -0.66, indicating substantial overestimation; in the later subperiod, mean $FE$ improved to -0.13, closer to zero. Crucially, uncertainty measures jumped dramatically: EPU mean nearly tripled from 126 (2011-2017) to 312 (2018-2025), and GPR also rose from 96 to 113. These increases underscore that the post-2018 period was characterized by unprecedented policy and geopolitical uncertainty. This sets the stage for understanding why a time-varying approach is necessary: the relationship between expectation updates and subsequent errors adapts to the macroeconomic environment.
...
This identification strategy goes beyond circular definition in three ways. First, by requiring a positive output response, we ensure the shock represents genuine economic news (an expansion) rather than spurious sentiment. Second, we impose the forecast error pattern for multiple horizons, not just on impact, ensuring persistence of the diagnostic mechanism. Third, as we show in the results, we achieve extremely high acceptance rates (over 99.95 percent), demonstrating this is not a rare artifact but a robust structural feature of the data. We also conduct robustness checks where we relax the output restriction or impose it only on impact, yielding quantitatively similar results.

This strategy aims to isolate a shock that is fundamentally expansionary but to which households overreact in their inflation forecasts. We draw 2,000 posterior draws of the VAR parameters. For each draw, we randomly sample orthonormal matrices $Q$ until we find one that satisfies the restrictions for the specified horizons (up to 10,000 attempts per draw). The shock is then normalized to increase $\mu_t$ by 1 percentage point on impact.
...
We observe several notable features in the $\beta_t$ path. First, $\beta_t$ is predominantly negative throughout the entire sample, with a posterior mean of approximately -0.52. For roughly 91 percent of the quarters, the 90\% credible interval for $\beta_t$ lies entirely below zero. This finding strongly rejects the rational expectations null of no predictable relationship and robustly supports the presence of diagnostic expectations.
...
Table \ref{tab:measurement_params} reports the posterior estimates for the static parameters in the measurement equation. The intercept $\alpha$ is -0.32, confirming the unconditional upward bias in household expectations. The coefficients on food inflation and M2 growth are negative, suggesting that periods of high food price growth or rapid money expansion are associated with subsequently more negative forecast errors, potentially because households over-extrapolate these signals. The PPI coefficient is negligible. Model comparison via the Deviance Information Criterion (DIC) strongly favors the time-varying parameter specification over a constant-$\beta$ model, validating our dynamic approach.
...
Additionally, we conduct model comparison using the Deviance Information Criterion (DIC). The time-varying parameter model yields a DIC of approximately 152.4, compared to 79.3 for a constant-$\beta$ model (Bayesian SSM without time variation) and 80.3 for a frequentist OLS baseline. While the TVP model has a higher DIC due to its increased complexity (effective parameters $p_{eff}=51$ vs. 6), it achieves a substantially better log-likelihood (-25.2 vs. -33.6), justifying the introduction of time variation in $\beta_t$ despite the added parameter count. A simpler TVP-SSM without EPU/GPR drivers yields DIC=151.8, suggesting the drift terms contribute marginally to model fit.
...
Our sign restriction algorithm achieves a high acceptance rate of 99.95 percent (Table \ref{tab:bvar_accept}), meaning that for virtually every posterior draw of the VAR (1,999 out of 2,000), there exists a structural shock satisfying our diagnostic criteria. This indicates that the pattern we are looking for is not a rare artifact but a robust feature embedded in the joint dynamics of the data.

\subsection{Impulse Response Analysis}
Figure \ref{fig:irf} presents the median impulse responses to a one unit (1 percentage point) diagnostic news shock, along with 68 percent credible bands. Table \ref{tab:bvar_irf} reports quantitative IRF values at selected horizons.

\textbf{Expectations and Inflation.} Expected inflation ($\mu_t$) shows a median increase of 0.36 standard deviations on impact (68\% band: [0.20, 0.50]). The response is persistent, remaining positive for about two years before fading (median at h=12: 0.02). In stark contrast, the response of actual inflation ($\pi_t$) is muted and uncertain. The median impact response is 0.07 standard deviations, with a wide credible band ([-0.14, 0.32]) that includes zero. By horizon 2, the median CPI response turns slightly negative (-0.03, band: [-0.15, 0.09]), and remains negligible thereafter. The key diagnostic signature—negative forecast errors—is confirmed: since expectations rise more than realized inflation, forecast errors $FE_h = \pi_{h+1} - \mu_h$ are negative for the first several quarters. Figure \ref{fig:fe_reversal} visualizes the cumulative forecast error path, which remains substantially negative through horizon 6-8 before partially reversing.

\textbf{Macroeconomic Context.} The identified shock occurs in a specific macroeconomic context. Industrial production growth ($g_t$) rises significantly, with a median impact response of 0.16 standard deviations (band: [-0.09, 0.40]), confirming the shock represents an economic expansion. Both the EPU and GPR indices \emph{decline} following the shock. EPU falls by a median of 0.17 standard deviations on impact (band: [-0.40, 0.08]), and remains below baseline for multiple quarters. GPR similarly drops, though with greater uncertainty (median: -0.03, band: [-0.41, 0.35]). These declines suggest that the diagnostic shock we identify corresponds to episodes of uncertainty resolution or positive surprises—moments when favorable news arrives and households overextrapolate the good developments. This interpretation aligns well with the theoretical notion that diagnostic expectations overweight salient recent signals: when good news arrives and uncertainty partially resolves, households become overly optimistic about sustained inflation.
...
Third, the Carlson-Parkin quantification assumes a fixed indifference threshold $\delta=0.5$ percentage points. Sensitivity analysis with $\delta \in \{0.3, 0.4, 0.5, 0.6, 0.7\}$ suggests that while the level of $\mu_t$ shifts with $\delta$, the cyclical correlation between forecast revisions and errors remains robust. This indicates our core diagnostic finding is not driven by the specific $\delta$ calibration.
