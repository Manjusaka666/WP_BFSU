...
\begin{enumerate}
  \item Estimate the restricted model (imposing $H_0\colon
    \beta = 0$) and obtain residuals
    $\hat{\varepsilon}_{ig}^{r}$ for each observation $i$ in
    cluster $g$.
  \item Draw a six-point random weight $w_g$ independently for
    each of the 31 province clusters.
    The weight takes one of six values, each with probability
    $1/6$.
  \item Construct the bootstrap error as
    $\varepsilon^*_{ig} = w_g \cdot \hat{\varepsilon}_{ig}^{r}$.
    All observations within a cluster receive the same weight,
    preserving the within-cluster correlation structure.
  \item Construct the bootstrap dependent variable:
    $y^*_{ig} = \hat{y}_{ig}^{r} + \varepsilon^*_{ig}$,
    where $\hat{y}_{ig}^{r}$ is the fitted value from the
    restricted model.
  \item Re-estimate the unrestricted model on
    $(y^*_{ig}, X_{ig})$ and compute the $t$-statistic $t^*_b$
    on the coefficient of interest.
  \item Repeat steps 2--5 for $B = 9{,}999$ replications.
  \item The bootstrap $p$-value is the fraction of replications
    for which $|t^*_b| \geq |t_{\text{obs}}|$, where
    $t_{\text{obs}}$ is the test statistic from the original
    data.
\end{enumerate}
...
\midrule
Eq 1 (Expectations) & Prov + Wave & 0.090 & 0.053
  & 0.100 & 0.088 & 90,133 \\
Eq 1 (Expectations) & Region$\times$Wave & 0.242 & 0.174
  & 0.174 & 0.237 & 90,133 \\
Eq 3 (Forecast error) & Prov + Wave & $-5.622$ & 1.182
  & $<0.001$ & $<0.001$ & 69,533 \\
Eq 3 (Forecast error) & Region$\times$Wave & $-6.067$ & 1.145
  & $<0.001$ & $<0.001$ & 69,533 \\
\bottomrule
\end{tabular}
