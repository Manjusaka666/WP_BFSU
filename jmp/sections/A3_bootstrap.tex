...
\begin{algorithmic}[1]
\Require Observed data $\{(y_{ig}, X_{ig})\}$; number of clusters $G = 31$; replications $B = 999$
...
Expectations & Province + region $\times$ wave & 0.119 & 0.050
  & 0.024 & 0.019 & 90,133 \\[4pt]
Forecast error & Province + wave & $-5.694$ & 1.197
  & $<0.001$ & $<0.001$ & 69,529 \\
Forecast error & Region $\times$ wave & $-6.291$ & 1.129
  & $<0.001$ & $<0.001$ & 69,529 \\
Forecast error & Province + region $\times$ wave & $-7.610$ & 1.124
  & $<0.001$ & 0.005 & 69,529 \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\footnotesize
\item Webb six-point distribution with 999 replications.
...
the bootstrap correction under all three fixed-effect structures.
The bootstrap $p$-values for Eq~3 are below $0.01$ in all three
specifications, and below $0.001$ in two of them.
The 95-percent bootstrap confidence intervals for
$\hat{\beta}_3$ exclude zero by a wide margin under all three
specifications.

The expectation coefficient $\hat{\beta}_1$ is marginal.
Under province-plus-wave FE, the bootstrap $p$-value of
$0.095$ is slightly below the conventional 10 percent
threshold, while the asymptotic $p$-value of $0.100$ is at
the boundary.
Under region-by-wave FE, both the asymptotic ($0.174$) and
bootstrap ($0.225$) $p$-values are above 10 percent.
...
problem.
The $t$-ratio for $\hat{\beta}_3$ exceeds 5 under all three
...
$\hat{\beta}_1$ under region-by-wave FE (from 0.174 to 0.225)
