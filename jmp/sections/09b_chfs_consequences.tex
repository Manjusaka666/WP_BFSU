...
%\label{subsec:chfs_consequences}
...
subsequent waves:
\begin{equation}%\label{eq:chfs_income}
...
power. First, the CHFS panel uses income rather than consumption as
the outcome, and income is a noisier proxy for the welfare channel.
Second, the overreactor classification relies on a single
cross-sectional wave with a coarse 5-point expectation scale, which
attenuates the signal. Third, sample attrition between 2011 and 2015
reduces the estimation sample from 6,640 to 5,550 households,
...
cross-sectional phenomenon (Section~\ref{subsec:chfs_info_channel})
