...
The variable is external to the survey, containing no
information derived from respondents.
It is cumulative: a household surveyed in 2020 experienced the
full path of meat-price movements since the 2018 wave.
And it varies at the province level, which allows me to absorb
region-by-wave fixed effects while retaining within-region
...
The NBS meat CPI index combines pork, beef, mutton, and
poultry, with pork receiving the dominant weight.
During the ASF episode, pork drove the bulk of the meat CPI
movement, so the index tracks the pork shock with high fidelity
during the identification window.
The cumulative measure also aggregates monthly observations
equally.
...
\begin{equation}\label{eq:eq3}
  ...
  + \varepsilon_{ipt},
\end{equation}
where $\alpha_p$ is a province fixed effect,
$\delta_{r(p)\times t}$ is a region-by-wave fixed effect,
and $\mathrm{FE}_{ipt}
...
ordinal expectation ($\mu_{ipt} \in \{-1, 0, 1\}$).
...
The preferred specification includes province fixed effects
together with region-by-wave fixed effects.
Province fixed effects absorb any time-invariant provincial
characteristic (income level, food culture, baseline inflation
expectations); region-by-wave fixed effects absorb
time-varying shocks common to provinces within a region.
The identifying variation is the within-province deviation
of the meat shock from the region-by-wave mean.
This double-differencing design asks whether a province that
receives a larger-than-regional-average meat shock in a given
wave produces a more negative forecast error than the same
province does at other times.
...
The decomposition also provides a scaling-free magnitude test.
Because $\beta_3 \approx \beta_2 - k\,\beta_1$, setting
$k = 0$ (ignoring the ordinal expectation entirely) yields
$\beta_3 = \beta_2$.
If $|\hat{\beta}_2|$ exceeds the rational bound, the
conclusion holds without assigning any cardinal meaning to
the ordinal codes.
Under the time-aggregated bound
(Section~\ref{sec:time_aggregation}),
$|\beta_3^R| \leq 24\omega \approx 1.20$.
Finding $|\hat{\beta}_2| = 5.25$ in the preferred
specification exceeds this bound by a factor of four,
independent of any scaling assumption.
...
ASF provides a biologically determined supply shock: the
disease entered China through contaminated feed imports and
spread along pig transportation corridors, with provincial
infection severity determined by initial herd density and
proximity to early outbreak sites.
The commodity-placebo test in Section~\ref{sec:mechanism}
provides further discipline: grain shocks of comparable
magnitude do not produce the same forecast-error pattern.
...
Province fixed effects absorb permanent provincial
characteristics: income, food culture, media environment,
baseline inflation expectations.
Region-by-wave fixed effects absorb any time-varying shock
common to provinces within a region, including aggregate
monetary policy, regional business cycles, and region-specific
food supply disruptions.
Together, the two sets of fixed effects leave only
within-province, within-region-wave variation in meat-price
shocks as the identifying source.
Robustness checks in Section~\ref{sec:robustness} show that
results are stable across alternative fixed-effect structures,
including region-by-wave alone and province-plus-wave.
...
the \citet{Webb2023} six-point weight distribution with 999
replications.
