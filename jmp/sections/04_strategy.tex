...
It is external to the survey: it contains no information
derived from respondents themselves.
It is cumulative: a household surveyed in 2020 experienced
...
The ASF episode provides a biologically determined source of
supply variation: the disease spread was driven by initial herd
density and animal movement networks, not by province-level
aggregate demand.
The commodity-placebo test in Section~\ref{sec:placebo}
then shows that grain shocks of comparable magnitude do not produce
the same expectation response, which is inconsistent with a generic
