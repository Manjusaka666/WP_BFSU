...
\input{../outputs/tables/robustness_alt_fe}

\input{../outputs/tables/robustness_inference}

\input{../outputs/tables/robustness_sample}
...
The two-sided randomization-inference $p$-value is below 0.01
for the forecast-error coefficient, while the expectation
coefficient is not statistically significant under randomization
inference.
...
from $-5.69$ (province-plus-wave) to $-6.29$
(region-by-wave) to $-7.61$ (province-plus-region-by-wave),
...
specification ($\hat{\beta}_1 = 0.119$, $p = 0.019$), where
...
69,529 (Eq~3) observations, but the identifying signal comes
...
$\hat{\beta}_3 = -6.29$ ($p < 0.01$, 93 cells), confirming
...
($p = 0.013$) and $\hat{\beta}_3 = -6.29$ ($p < 0.01$),
...
at a time.
The coefficient ranges from $-5.87$ (dropping West) to
$-6.88$ (dropping East), with $p < 0.01$ in all six
subsamples.
...
The estimated $\hat{\beta}_3 = -7.61$ exceeds even this
