...
Usage examples:
  # Crawl the first 25 list pages and download/parse PDFs
...
Outputs:
  - data/intermediate/pboc_depositor_survey_quarterly.csv  (wide; indices + shares)
  - data/intermediate/pboc_infl_exp_index.csv              (backward-compatible; price index only)
...
# Web locations (PBOC)
# ----------------------------
BASE_LIST_ROOT = "https://www.pbc.gov.cn/diaochatongjisi/116219/116227/"
LIST_PAGE_FMT = urljoin(BASE_LIST_ROOT, "11874-{page}.html")  # pagination
LIST_PAGE_1 = LIST_PAGE_FMT.format(page=1)  # first page

# ----------------------------
# Regex: article list parsing
# ----------------------------
TITLE_FILTER = "城镇储户问卷调查报告"

# ----------------------------
# Regex: extract "report quarter" from title
# ----------------------------
_TITLE_Q_RE = re.compile(r"(?P<year>\d{4})年(?P<qcn>第[一二三四]季度)")

_QMAP = {"第一季度": "Q1", "第二季度": "Q2", "第三季度": "Q3", "第四季度": "Q4"}
...
def main():
    ...
    ap.add_argument(
        "--out_wide", default="data/intermediate/pboc_depositor_survey_quarterly.csv"
    )
    ap.add_argument(
        "--out_price_index", default="data/intermediate/pboc_infl_exp_index.csv"
    )
